...
<div class="title">
    ...
    <div class="caoZuo"><div class="manageDiv">
        <div class="leftManageDiv">
            <ul>
                <li onclick="huanyingyong(0);">留言管理</li>
                <li onclick="huanyingyong(1),message('<?php echo (session('rtype')); ?>');">用户管理</li>
                <a href="<?php echo U('Manage/manage');?>"><li>回管理中心</li></a>
            </ul>
        </div>
        <div id="manage">
            <div class="lunManage">
                <p class="lunM" style="background-color: #808080;color: white;font-weight: bold">留言管理中心</p>
            <span class="lunL">
            <table class="table" cellspacing="0" cellpadding="0">
                <tr><td style="display: none">留言编号</td><td>用户头像</td><td>用户昵称</td><td style="width: 100px">留言主题</td><td>留言时间</td><td>用户IP</td><td>操作</td></tr>
                <?php if(is_array($ShowMessData)): $i = 0; $__LIST__ = $ShowMessData;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$showMess): $mod = ($i % 2 );++$i;?><tr>
                    <td style="display: none">24</td>
                    <td style="padding: 0px;width: 60px">
                        <?php if(($showMess["upic"]) == "NULL"): ?><img src="/mybbs/Public/Uploads/001.gif" width="40" height="30" />
                            <?php else: ?>
                            <img src="/mybbs/Public/Uploads/<?php echo ($showMess["upic"]); ?>" width="40" height="30"><?php endif; ?>
                    </td>
                    <td><?php echo ($showMess["sname"]); ?></td>
                    <td><?php echo ($showMess["mbtitle"]); ?></td>
                    <td><?php echo ($showMess["mbdate"]); ?></td>
                    <td><?php echo ($showMess["mbip"]); ?></td>
                    <td>
                        <a href="<?php echo U('Manage/UserMessageDel');?>?mbid=<?php echo ($showMess["mbid"]); ?>"><span>删除话题</span></a>
                        /
                        <a href="<?php echo U('Manage/TuiJianMessage');?>?mbid=<?php echo ($showMess["mbid"]); ?>"><span>设为推荐</span></a>
                    </td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
            </table>
            </span>
            </div>
            <div class="userManage">
                <p class="userM"  style="background-color: #808080;color: white">用户管理中心</p>
            <span class="userL">
            <table class="table" cellspacing="0" cellpadding="0">
                <tr><td>用户名</td><td>用户头像</td><td style="width: 100px">用户昵称</td><td>用户Email</td><td>用户手机</td><td>用户QQ</td><td>操作</td></tr>
                <?php if(is_array($userManage)): $i = 0; $__LIST__ = $userManage;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?><tr><td><?php echo ($user["uname"]); ?></td>
                        <td style="padding: 0px;width: 60px">
                            <?php if(($user["upic"]) == "NULL"): ?><img src="/mybbs/Public/Uploads/001.gif" width="40" height="30" />
                                <?php else: ?>
                                <img src="/mybbs/Public/Uploads/<?php echo ($user["upic"]); ?>" width="40" height="30"><?php endif; ?>
                        </td>
                        <td><?php echo ($user["sname"]); ?></td>
                        <td><?php echo ($user["uemail"]); ?></td>
                        <td><?php echo ($user["uphone"]); ?></td>
                        <td><?php echo ($user["uqq"]); ?></td>
                        <td>
                            <span class="quanXian"><a href="<?php echo U('Manage/setrole');?>?uid=<?php echo ($user["uid"]); ?>">权限管理</a></span>
                        </td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
            </table>
            </span>
            </div>
        ...
        <script type="text/javascript" src="/mybbs/Public/js/manage.js"></script>
    </div>
