...
<div class="title">
    ...
    <div class="caoZuo"><div class="quanTitle">用户权限设置</div>
        <div class="quanxian">
            <div style="width: 300px;float: left;margin-left: 150px;margin-top: 40px">
               <?php if(is_array($UserQX)): $i = 0; $__LIST__ = $UserQX;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$QX): $mod = ($i % 2 );++$i;?>给:
                   <?php if(($QX["upic"]) == "NULL"): ?><img src="/mybbs/Public/Uploads/001.gif" width="20" height="20">
                       <?php else: ?>
                       <img src="/mybbs/Public/Uploads/<?php echo ($QX["upic"]); ?>" width="20" height="20"><?php endif; ?>
                   <?php echo ($QX["sname"]); endforeach; endif; else: echo "" ;endif; ?>
                <form action="server/server.php" method="post" name="setRoleForm">

                设置组:
                    <?php if(is_array($UserQX)): $i = 0; $__LIST__ = $UserQX;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$QX): $mod = ($i % 2 );++$i;?><input type="hidden" value="<?php echo ($QX["uid"]); ?>" name="uid"/><?php endforeach; endif; else: echo "" ;endif; ?>
                <select id="type" name="rtype">
                    <option value="a">管理员</option>
                    <option value="b">版主</option>
                </select>
                请选择权限:
                <select id="xuan" multiple="multiple" name="atype[]">
                ...
                <br>
                <input type="submit" name="setRole" value=" 确定" />
            </form>
            </div>
                <script type="text/javascript" src="/mybbs/Public/js/messageUser.js"></script>
        </div>
    </div>
