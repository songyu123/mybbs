...
<div class="title">
    ...
    <div class="caoZuo">
        <div class="userDiv" style="position: relative">
        <div class="huiYuanDatum"><span style="margin-left: 350px;cursor: pointer;font-size: 20px;color: white">会员用户资料修改</span></div>
        <!--注册表单-->
        <form action="<?php echo U('User/update');?>" method="post" name="fromDatum" enctype="multipart/form-data">
        <table id="userDatum">
            <?php if(is_array($user)): $i = 0; $__LIST__ = $user;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr><td align='right'class="td">用户名:</td><td class="tds" colspan="3"><input disabled="false"  type="text"  value="<?php echo ($vo["uname"]); ?>"/></td></tr>
            <tr><td align='right'class="td">昵 称:</td><td class="tds"  colspan="3"><input name="sname" id="userDatumSname" type="text" value="<?php echo ($vo["sname"]); ?>"/></td></td></tr>
            <tr><td align='right'class="td">密 码:</td><td class="tds"  colspan="3"><input name="upwd" type="password" value="<?php echo ($vo["upwd"]); ?>" id="userDatumPwd"/></td></td></tr>
            <tr><td align='right'class="td">Email:</td><td class="tds"  colspan="3"><input name="uemail" type="text" id="userDatumEmail" value="<?php echo ($vo["uemail"]); ?>"/></td></tr>
            <tr><td align='right'class="td">电 话:</td><td class="tds"  colspan="3"><input name="uphone" type="text" id="userDatumPhone" value="<?php echo ($vo["uphone"]); ?>"/></td></tr>
            <tr><td align='right'class="td">QQ:</td><td class="tds"  colspan="3"><input name="uqq" type="text" id="userDatumQQ" value="<?php echo ($vo["uqq"]); ?>"/></td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
        </table>
        <!--头像上传-->
            <div class="userPic">
                <div class="userPicTitle">头像上传</div>
                <div id="userPicImgUp">
                    <?php if(is_array($user)): $i = 0; $__LIST__ = $user;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if(($vo["upic"]) == "NULL"): ?><img src="/mybbs/Public/Uploads/001.jpg" width="130" height="130">
                        <?php else: ?>
                       <img src="/mybbs/Public/Uploads/<?php echo ($vo["upic"]); ?>" width="130" height="130"/><?php endif; endforeach; endif; else: echo "" ;endif; ?>
                </div>
                <div class="imgList">
                    <input class="picInput" type="file" name="userPic"/>
                </div>
            </div>
            <input style="position: absolute;top: 400px;left: 450px" class="subs" type="submit"name="subDatum" value="确认修改" />
        </form>
    </div>
    </div>
